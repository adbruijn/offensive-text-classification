...
from train import train_model, train_model_features
from evaluate import evaluate_model, evaluate_model_features
from utils import accuracy_recall_precision_f1, save_checkpoint, load_checkpoint
from data_loader import get_data, get_data_bert, get_data_features
import models
...
def train_and_evaluate(num_epochs, model, optimizer, loss_fn, train_dataloader, val_dataloader, early_stopping_criteria, directory, use_bert, use_mongo):
    ...
    for epoch in trange(num_epochs, desc="Epoch"):
        ...
        ### TRAINING ###
        train_results = train_model_features(model, optimizer, loss_fn, train_dataloader, device, use_bert)
        ...
        ### EVALUATION ###
        val_results = evaluate_model_features(model, optimizer, loss_fn, val_dataloader, device, use_bert)
...
def main(output_dim,
    ...
    if use_bert:
        ...
        embedding_dim, vocab_size, embedding_matrix, train_dataloader, val_dataloader, test_dataloader = get_data_features(int(max_seq_length), embedding_file=embedding_file, batch_size=batch_size)
...
# #
# # #USER
# # print("USER:", len(re.findall("USER",text)))
# #
# # #Punctuation
# #
# # #Upper case
